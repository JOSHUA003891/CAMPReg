...
function insertData($conn, $u_name, $u_email)
{
    ...
    elseif (!filter_var($u_email, FILTER_VALIDATE_EMAIL)) {
        ...
        $check_email = mysqli_query($conn, "SELECT `email` FROM `loginDetails` WHERE `email` = '$u_email'");
        ...
        $query = mysqli_query($conn, "INSERT INTO `loginDetails`(`name`,`email`) VALUES('$u_name','$u_email')");
        // IF USER INSERTED
        if ($query) {
            return true;
        }
